...
def predict():
    ...
    fig.update_layout(
                ...
                font=dict(size=17),
                yaxis={'side': 'right'} ,
                title= '<b>{} Forecast for {}/USD</b>'.format(model_display[model_sel], crypto.symbol),
                title_x=0.5,
...
def index_page():
    st.markdown("<h1 style='text-align: center; color: white;'>Welcome To Crypto Forecast</h1>", unsafe_allow_html=True)
...
state = window_selection_c.radio('',('Home', 'Forecast'))
...
if state == 'Home':
    ...
    window_selection_c.write('1. Select currency to forecast')
    window_selection_c.write('2. Select model to use to forecast')
    window_selection_c.write('3. Make your own informed decision')
